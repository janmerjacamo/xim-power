...
containerType.addEventListener('change', () => { currentContainer = containerType.value; calculateDistribution(); });
addItemBtn.addEventListener('click', () => {
    ...
    if (!L || !W || !H || !qty || !weight) { alert('Complete todas las dimensiones'); return; }
    items.push({ id: items.length + 1, name, L, W, H, qty, weight, orientation: orientation === 'auto', volume: (L * W * H * qty) / 1e9 });
    updateItemsTable();
...
function removeItem(index) { items.splice(index, 1); updateItemsTable(); calculateDistribution(); }
function updateItemsTable() {
    if (items.length === 0) { itemsBody.innerHTML = '<tr><td colspan="5" style="text-align:center">No hay productos cargados</td></tr>'; return; }
    itemsBody.innerHTML = items.map((item, idx) => `<tr><td><strong>${item.name}</strong></td><td>${item.L} x ${item.W} x ${item.H}</td><td>${item.qty}</td><td>${(item.weight * item.qty).toFixed(0)} kg</td><td><button onclick="removeItem(${idx})" style="background:#DC2626; color:white; border:none; padding:4px 8px; border-radius:6px; cursor:pointer"><i class="fas fa-trash"></i></button></td></tr>`).join('');
...
function calculateDistribution() {
    ...
    let currentX = 0, currentY = 0, currentZ = 0, maxRowHeight = 0, currentRowWidth = 0;
    for (const item of sortedItems) {
        for (let q = 0; q < item.qty; q++) {
            ...
            if (item.orientation) {
                ...
                for (const o of orientations) {
                    ...
                    if (l_m <= container.L && w_m <= container.W && h_m <= container.H) {
                        const fit = Math.abs(container.L - currentX - l_m) + Math.abs(container.W - currentY - w_m) + Math.abs(container.H - currentZ - h_m);
            ...
            placements.push({ name: item.name, l, w, h, x: currentX, y: currentY, z: currentZ, color: `hsl(${(item.id * 47) % 360}, 70%, 55%)` });
    ...
    if (occupancy > 95) showAlert('warning', '⚠️ Ocupación >95%. Considere contenedor más grande');
    else if (weightPercent > 90) showAlert('warning', '⚠️ Peso cercano al límite');
    else showAlert('success', '✅ Distribución óptima calculada');
    drawPlot(placements, container);
}
function showAlert(type, message) {
    alertsContainer.innerHTML = `<div class="alert alert-${type}"><i class="fas fa-${type === 'warning' ? 'exclamation-triangle' : 'check-circle'}"></i> ${message}</div>`;
...
function drawPlot(placements, container) {
    ...
    traces.push({ type: 'scatter3d', mode: 'lines', x: [x[0], x[1], x[2], x[3], x[0], x[4], x[5], x[6], x[7], x[4]], y: [y[0], y[1], y[2], y[3], y[0], y[4], y[5], y[6], y[7], y[4]], z: [z[0], z[1], z[2], z[3], z[0], z[4], z[5], z[6], z[7], z[4]], line: { color: '#1E293B', width: 3 }, name: 'Contenedor' });
    placements.forEach(p => {
        const x0 = p.x, x1 = p.x + p.l, y0 = p.y, y1 = p.y + p.w, z0 = p.z, z1 = p.z + p.h;
        traces.push({ type: 'mesh3d', x: [x0, x1, x1, x0, x0, x1, x1, x0], y: [y0, y0, y1, y1, y0, y0, y1, y1], z: [z0, z0, z0, z0, z1, z1, z1, z1], opacity: 0.7, color: p.color, name: p.name, hoverinfo: 'text', text: `${p.name}<br>${(p.l*1000).toFixed(0)}x${(p.w*1000).toFixed(0)}x${(p.h*1000).toFixed(0)} mm` });
    });
    const layout = { scene: { xaxis: { title: 'Largo (m)', range: [0, container.L + 0.2] }, yaxis: { title: 'Ancho (m)', range: [0, container.W + 0.2] }, zaxis: { title: 'Alto (m)', range: [0, container.H + 0.2] }, aspectmode: 'manual', aspectratio: { x: container.L, y: container.W, z: container.H }, camera: { eye: { x: 1.8, y: 1.5, z: 1.2 } } }, margin: { l: 0, r: 0, b: 0, t: 30 }, title: { text: `${container.name} - ${placements.length} unidades`, font: { size: 14 } } };
    Plotly.newPlot('plot', traces, layout, { responsive: true });
}
resetViewBtn.addEventListener('click', () => { Plotly.relayout('plot', { 'scene.camera': { eye: { x: 1.8, y: 1.5, z: 1.2 } } }); });
exportReportBtn.addEventListener('click', () => {
    ...
    items.forEach(item => { report += `${item.name}: ${item.qty} uds (${item.L}x${item.W}x${item.H} mm, ${item.weight} kg)\n`; });
    report += `\n--- RESUMEN ---\nTotal unidades: ${totalUnits.textContent}\nPeso total: ${totalWeight.textContent} kg / ${container.maxWeight} kg\nVolumen: ${totalVolume.textContent} m³ / ${(container.L*container.W*container.H).toFixed(2)} m³\nOcupación: ${occupancyRate.textContent}\n`;
...
clearAllBtn.addEventListener('click', () => { if (confirm('¿Eliminar todos los productos?')) { items = []; updateItemsTable(); calculateDistribution(); } });
